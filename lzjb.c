      ...
      *copymap |= copymask;
      for (mlen = MATCH_MIN; mlen < MATCH_MAX; mlen++)
        if (src[mlen] != cpy[mlen])
          break;
      *dst++ = ((mlen - MATCH_MIN) << (NBBY8 - MATCH_BITS)) | (offset >> NBBY8);
      *dst++ = (uchar_t) offset;
      src += mlen;
    } else {
      *dst++ = *src++;
...
size_t decompress(uchar_t *s_start, uchar_t *d_start, size_t s_len, size_t d_len) {
  uchar_t *src = s_start;
  uchar_t *dst = d_start;
  ...
  while (src < s_end && dst < d_end) {
    ...
    if (copymap & copymask) {
      int mlen = (src[0] >> (NBBY8 - MATCH_BITS)) + MATCH_MIN;
      int offset = ((src[0] << NBBY8) | src[1]) & OFFSET_MASK;
      src += 2;
...
uint32_t checksum32(uchar_t *cp_arg, size_t length) {
  uchar_t *cp, *ep;
  uint32_t sum = 0;
...
cat scherz_20141201.log | grep -v -e 0x8037 -e 0x8032 -e 0x801E -e 0x8036 -e 0x8026 -e 0x8023 -e 0x8029 -e 0x800D -e 0x801B -e 0x801F -e 0x8005 -e 0x0004 -e 0x801D -e 0x802E -e 0x8028 -e 0x000E -e 0xFFFF -e 0x0000


28*1392+15*96*120+225*2+127*2+20

cat niq_log_defs.inc | sed 's/^.*"\[/\[/;s/")//' | wc -c
sed -i 's/[^0-9. ]//g;s/     / /g;s/    / /g;s/   / /g;s/  / /g;s/^ //;s/ $//;' scherz_20141201.log.modified2
...
int main(int argc, char *argv[]) {
  ...
  if(argc != 2) {
    ...
    while((s_len = fread(s_buffer, 1, BUFFER_SIZE, s_file)) != 0) {
      ...
      //printf ("%s", s_buffer);
      //printf("%d\n", d_len);
      fwrite(d_buffer, 1, d_len, d_file);
      
      int u_len = decompress((uchar_t *) d_buffer, (uchar_t *) u_buffer, d_len, BUFFER_SIZE+1);
